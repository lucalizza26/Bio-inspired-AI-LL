...
HYPERPARAMS["lr"] = 1e-4
HYPERPARAMS["epsilon_decay"] = 0.002


end = 1000
start = 51 # one more than the last saved model to avoid overwrites


for i in range(start,end+1):
    print(f"Testing model no. {i}")
    CSV_FILENAME = f"consistency_analysis/lunar_lander_final_{i}.csv"
    ...
    train_dqn(env, policy_net, target_net, optimizer, replay_buffer, writer,
            HYPERPARAMS["batch_size"], HYPERPARAMS["gamma"], HYPERPARAMS["target_update_freq"],
                saving_freq=100, csv_filename=CSV_FILENAME, max_episodes=2000, iteration=i)
