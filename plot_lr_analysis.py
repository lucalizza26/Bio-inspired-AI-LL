...
lrs = [1e-05, 5e-05, 1e-4, 5e-4, 1e-3, 5e-03, 1e-2, 5e-2, 1e-1, 5e-1]
...
# for i in lrs:
#     csv_filename = f"lr_analysis/lunar_lander_lr_{str(i)}.csv"
#     new_csv_filename = f"lr_analysis/model_lr_{str(i)}.csv"
...
for lr in lrs:
    csv_filename = f"lr_analysis/model_lr_{str(lr)}.csv"
    data[str(lr)] = {
        "lr": lr,
...
def convergence(lr=str,):
    ...
    for i in range(len(ep)):
        ...
        if successes >= 95 and mean >= 180:
            print(f"LR {lr} converged at episode {ep[i]} - mean: {mean}, std: {std}")
            data[lr]["converged"] = True
            data[lr]["converged_episode"] = ep[i]
            data[lr]["mean"] = mean
            data[lr]["std"] = std
            return True
    return False
...
for lr in data:
    if not convergence(lr):
        print(f"LR {lr} did not converge.")
        data[lr]["converged"] = False
        data[lr]["converged_episode"] = None
        data[lr]["mean"] = None
        data[lr]["std"] = None
...
for lr in data:
    if data[lr]["converged"]:
        x.append(str(data[lr]["lr"]))
        y.append(data[lr]["converged_episode"])
        w.append(data[lr]["mean"])
        z.append(data[lr]["std"])
...
x = ["1e-5", "5e-5", "1e-4", "5e-4", "1e-3", "5e-3", "1e-2", "5e-2", "1e-1", "5e-1"]
